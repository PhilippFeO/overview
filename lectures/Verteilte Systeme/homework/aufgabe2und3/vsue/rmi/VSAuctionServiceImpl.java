...
public class VSAuctionServiceImpl implements VSAuctionService {
    ...
    // TODO Mehrere gleichzeitige Aufrufe per „synchronized“ ausreichend behandelt?
    ...
    */
    public static void superviseAuctions(){
        ...
        // Datenstruktor, um zu löschende Auktionen zu speichern, damit der Iterator nicht während der Iteration modifiziert wird.
        // Das Resultat ist nicht definiert, wenn man Elemente während der Iteration löscht 
        List<String> removeTheseAuctions = new ArrayList<String>();
        while(true){
            /*
            ...
            synchronized(VSAuctionServiceImpl.class){
                for(VSAuction auction: auctions.values()) {  // Auktionen sind eindeutig wegen ihres Namens
                    ...
                    if (duration == Integer.MIN_VALUE) {
                        ...
                        // Auktionsnamen, also den Schlüssel, speichern
                        removeTheseAuctions.add(auctionName);
                        continue;
                    }
                ...
                // Auktionen in einem zweiten Schritt löschen
                // Hier problemlos möglich, da man über eine andere Datenstruktur iteriert
                for(final String auction : removeTheseAuctions){
                    auctions.remove(auction);
                    buyers.remove(auction);
                    sellers.remove(auction);
                    durations.remove(auction);
                }
                // Liste mit den zu löschenden Auktion „leeren“
                removeTheseAuctions = new ArrayList<String>();
            }
            try {
                Thread.sleep(1000);
